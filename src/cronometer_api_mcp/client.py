...
class CronometerClient:
    ...
    def add_serving(
        ...
        # Cronometer's backend rejects measureId=0 for some database foods
        # (CRDB-sourced) with a confusing "userId is null" error. When the
        # caller does not specify a measure, fall back to the food's
        # defaultMeasureId so the request matches what the official app sends.
        if not measure_id:
            try:
                ...
                logger.warning(
                    "add_serving: defaultMeasureId lookup failed (%s); continuing with 0",
                    exc,
                )
